...
export function ProfileChart({ profile }: { profile: ProfileResult | null }) {
  ...
  return (
    <Panel action={<Activity className="text-slate-500" size={17} />} eyebrow="Centerline sample" title={profile ? `${profile.field} along ${profile.axis.toUpperCase()}` : "Line profile"}>
        ...
        <div className="p-3">
          <svg aria-label={`Line profile of ${profile.field} along ${profile.axis}, from ${minY} to ${maxY}.`} className="h-48 w-full" role="img" viewBox={`0 0 ${width} ${height}`}>
            {[0, 0.5, 1].map((tick) => {
              ...
              return <line key={tick} stroke="#22313d" x1={padding.left} x2={width - padding.right} y1={y} y2={y} />;
            })}
            <path d={path} fill="none" stroke="#2dd4bf" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" />
            <text fill="#7d8b98" fontSize="11" x={padding.left} y={height - 8}>{formatNumber(minX)}</text>
            <text fill="#7d8b98" fontSize="11" textAnchor="end" x={width - padding.right} y={height - 8}>{formatNumber(maxX)}</text>
            <text fill="#7d8b98" fontSize="11" x={6} y={padding.top + 4}>{formatNumber(maxY)}</text>
            <text fill="#7d8b98" fontSize="11" x={6} y={height - padding.bottom}>{formatNumber(minY)}</text>
          </svg>
        </div>
      ) : (
        <div className="grid min-h-48 place-items-center p-6 text-center text-sm text-slate-500">Create a profile from the controls or ask the assistant.</div>
      )}
    </Panel>
  );
}
